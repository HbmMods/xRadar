...
public class SoundLoopSiren extends SoundLoopMachine {
    ...
    public void update() {
        ...
        if (player != null) {
            f = (float) Math.sqrt(
                Math.pow(xPosF - player.posX, 2) + Math.pow(yPosF - player.posY, 2) + Math.pow(zPosF - player.posZ, 2));
            volume = func(f, intendedVolume);
        } else {
            volume = intendedVolume;
        }

        if (te instanceof TileEntityMachineSiren && !te.isInvalid()
            && te.getWorldObj()
                .getBlock(te.xCoord, te.yCoord, te.zCoord) == ModBlocks.machine_siren
            && te.getWorldObj()
                .isBlockIndirectlyGettingPowered(te.xCoord, te.yCoord, te.zCoord)) {
            this.setRepeat(
                type.name()
