...
public class Clowder {
    ...
    public boolean addMember(World world, String name) {
        ...
        if (world.getPlayerEntityByName(name) == null) return false;

        if (inverseMap.containsKey(name) || members.get(name) != null) return false;

        members.put(name, time());
        inverseMap.put(name, this);

        ClowderData.getData(world)
            .markDirty();
    ...
    public boolean transferOwnership(World world, String key) {
        ...
        if (members.get(key) == null) return false;
    ...
    public int getPermLevel(String name) {
        ...
        if (this.leader.equals(name)) return 3;

        if (this.officers.contains(name)) return 2;

        if (this.members.get(name) != null) return 1;
    ...
    public String getDecoratedName() {
        ...
        if (getPrestige() < 25) n += "";
        else if (getPrestige() < 50) n += " ★";
        else if (getPrestige() < 75) n += " ★★";
        else if (getPrestige() < 100) n += " ★★★";
        else if (getPrestige() < 125) n += " ★★★★";
        else if (getPrestige() < 150) n += " ★★★★★";
        else if (getPrestige() < 250) n += " ✪";
        else if (getPrestige() < 500) n += " ✪✪";
        else if (getPrestige() < 750) n += " ✪✪✪";
        else if (getPrestige() < 1000) n += " ✪✪✪✪";
        else if (getPrestige() < 10000) n += " ✪✪✪✪✪";
        else n += " [✶]";
    ...
    public int tryAddWarp(EntityPlayer player, int x, int y, int z, String name) {
        ...
        if (!ClowderTerritory.isPlayerHome(player)) return 1;
        ...
        for (int i = 2; i <= 5; i++) {
            ...
            if (block == ModBlocks.tp_tent) {
                ...
                int[] pos = ((BlockDummyable) ModBlocks.tp_tent)
                    .findCore(world, x + dir.offsetX * 2, y, z + dir.offsetZ * 2);
    ...
    public boolean isRaidable() {
        ...
        if (MainRegistry.freeRaid) return true;
        ...
        for (String s : this.members.keySet()) {
            ...
            if (l > System.currentTimeMillis()) online++;
        }

        if (members >= 6) return online >= 3;

        if (members >= 3) return online >= 2;
    ...
    public void addPrestigeGen(float f, World world) {
        ...
        if (prestigeGen < 0) prestigeGen = 0F;
    ...
    public void addPrestigeReq(float f, World world) {
        ...
        if (prestigeReq < 0) prestigeReq = 0F;
    ...
    public void saveClowder(int i, NBTTagCompound nbt) {
        ...
        /// SAVE MEMBERS ///
        for (int j = 0; j < this.members.keySet()
            .size(); j++)
            nbt.setString(
                i + "_" + j,
                (String) this.members.keySet()
                    .toArray()[j]);
        ...
        /// SAVE WARPS ///
        for (int j = 0; j < this.warps.keySet()
            .size(); j++) {

            String name = (String) this.warps.keySet()
                .toArray()[j];
    ...
    public static Clowder loadClowder(int i, NBTTagCompound nbt) {
        ...
        if (c.uuid.isEmpty()) c.uuid = UUID.randomUUID()
            .toString();
        ...
        for (int j = 0; j < count; j++) c.members.put(nbt.getString(i + "_" + j), time());

        for (int j = 0; j < co; j++) c.officers.add(nbt.getString(i + "_" + j + "_off"));
    ...
    public void notifyCapture(World world, int x, int z, String type) {
        ...
        notifyAll(
            ...
            new ChatComponentText(
                EnumChatFormatting.RED + "One of your " + type + " at X:" + x + " / Z:" + z + " is under attack!"));
        ...
        if (!warps.isEmpty()) {
            ...
            notifyAll(
                world,
                new ChatComponentText(
                    EnumChatFormatting.RED + "Your closest warp is " + closest + " (" + ((int) dist) + "m)"));
        }
    }
    ...
    public static void readFromNBT(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < count; i++) clowders.add(loadClowder(i, nbt));
    ...
    public static void writeToNBT(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < clowders.size(); i++) clowders.get(i)
            .saveClowder(i, nbt);
    }
    ...
    public static Clowder getClowderFromName(String name) {
        ...
        for (Clowder clowder : clowders) {
            if (clowder.name.toLowerCase()
                .equals(name)) return clowder;
        }
    ...
    public static Clowder getClowderFromUUID(String uuid) {
        ...
        for (Clowder clowder : clowders) {
            if (clowder.uuid.toLowerCase()
                .equals(uuid)) return clowder;
        }
