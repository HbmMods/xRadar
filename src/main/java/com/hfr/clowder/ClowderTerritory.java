...
public class ClowderTerritory {
    ...
    public static Ownership getOwner(int x, int z) {
        ...
        if (meta == null) return WILDERNESS;

        Ownership owner = meta.owner;

        if (owner.zone == Zone.FACTION && owner.owner == null) return WILDERNESS;
    ...
    public static boolean isPlayerHome(EntityPlayer player) {
        ...
        if (clowder == null) return false;

        Ownership owner = getOwnerFromInts((int) player.posX, (int) player.posZ);

        if (owner != null && owner.zone == Zone.FACTION && owner.owner == clowder) return true;
    ...
    public static TerritoryMeta getMetaFromInts(int x, int z) {
        ...
        if (meta != null && meta.owner.zone == Zone.FACTION && meta.owner.owner == null) meta.owner = WILDERNESS;

        if (meta != null && meta.owner == WILDERNESS) return null;
    ...
    public static class Ownership {
        ...
        public Ownership(Zone zone, Clowder owner) {
            ...
            if (zone == Zone.FACTION) this.owner = owner;
        }
        ...
        public void writeToNBT(NBTTagCompound nbt, String code) {
            ...
            if (zone == Zone.FACTION && owner == null) return;

            nbt.setInteger("ownership_" + code + "_zone", zone.ordinal());

            if (zone == Zone.FACTION) nbt.setString("ownership_" + code + "_owner", owner.name);
        }
        ...
        public static Ownership readFromNBT(NBTTagCompound nbt, String code) {
            ...
            if (zone == Zone.FACTION && clowder == null) return WILDERNESS;
    ...
    public static class TerritoryMeta {
        ...
        // chunks will persist if there's an operational flag within its bounds or if the supposedly flag-bearing chunk
        // is not loaded
        public boolean checkPersistence(World world, CoordPair claim) {

            if (owner.zone != Zone.FACTION) return true;

            if (flagY < 0) return false;

            Clowder own = owner.owner;
            CoordPair origin = getCoordPair(flagX, flagZ);

            if (world == null || world.getChunkProvider() == null) return true;

            Chunk chunk = world.getChunkProvider()
                .provideChunk(flagX >> 4, flagZ >> 4);
            ...
            if (chunk != null) {
                ...
                if (te instanceof ITerritoryProvider) {
                    ...
                    if (flag.getOwner() != own) {
                        ...
                        if (flag instanceof TileEntityFlag && ((TileEntityFlag) flag).height > 0
                            && ((TileEntityFlag) flag).height < 1.0F) return true;

                        if (flag instanceof TileEntityConquerer && ((TileEntityConquerer) flag).height > 0) return true;
    ...
    // i called it an automaton because it mindlessly iterates through the persistence checks without the common handler
    ...
    public static void readFromNBT(NBTTagCompound nbt) {
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (meta != null && meta.owner.zone != Zone.WILDERNESS) territories.put(code, meta);
        }
    }
