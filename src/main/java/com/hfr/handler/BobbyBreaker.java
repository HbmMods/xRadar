...
public class BobbyBreaker {
    ...
    public static int getResistanceValue(Block b, int meta) {
        ...
        HashMap<Integer, Integer> map = resistance.get(
    ...
    public static void handleExplosionEvent(Detonate event) {
        ...
        if (!world.isRemote && event.explosion.isSmoking) {
            ...
            List<ForgeDirection> offsets = new ArrayList() {
            ...
            // randomly offset the proposed explosion location to find space, starting with no offset (best-case
            // scenario)
            for (int i = 0; i < offsets.size(); i++) {
                ...
                if (world.getBlock(ix, iy, iz) == Blocks.air) {
                    BobbyExplosion explosion = new BobbyExplosion(
                        world,
