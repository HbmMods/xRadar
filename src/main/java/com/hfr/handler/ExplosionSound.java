...
public class ExplosionSound {
    ...
    public static void handleExplosion(World world, Explosion explosion) {
        ...
        if (pow > max) pow = (float) max;

        double farRange = 1000D * pow / max;
        PacketDispatcher.wrapper.sendToAllAround(
            new ExplosionSoundPacket((int) x, (int) y, (int) z, pow),
            new TargetPoint(world.provider.dimensionId, x, y, z, farRange));
    ...
    public static void handleClient(EntityPlayer player, int x, int y, int z, float pow) {
        ...
        double distance = Math
            .sqrt(Math.pow(player.posX - x, 2) + Math.pow(player.posY - y, 2) + Math.pow(player.posZ - z, 2));
