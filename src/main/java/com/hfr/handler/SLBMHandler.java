...
public class SLBMHandler {
    ...
    public static void tryLaunch(World world, EntityPlayer player, int xCoord, int zCoord) {
        ...
        if (!world.isRemote && hasSLBM(player)) {
            ...
            if (player.inventory.hasItem(missile) && entVehicle.getEntityData()
                .getLong("slbmLastShot") + delay < System.currentTimeMillis()) {
                ...
                if (dist <= range) {
                    ...
                    switch (type) {
                        // Martin-2
                        case 1:
                            EntityMissileMartin martin = new EntityMissileMartin(
                                world,
                                posX,
                                posY,
                                posZ,
                        ...
                        // Tempest
                        case 2:
                            EntityMissilePegasus tempest = new EntityMissilePegasus(
                                world,
                                posX,
                                posY,
                                posZ,
                        ...
                        // Spear
                        case 3:
                            EntityMissileSpear spear = new EntityMissileSpear(
                                world,
                                posX,
                                posY,
                                posZ,
                        ...
                        // Pepper
                        case 4:
                            EntityMissileShell pepper = new EntityMissileShell(
                                world,
                                posX,
                                posY,
                                posZ,
                    ...
                    entVehicle.getEntityData()
                        .setLong("slbmLastShot", System.currentTimeMillis());
                    return;
                }
            }
    ...
    public static Item getMissile(int flightType, int warhead) {
        ...
        if (flightType == 1) {
            ...
            if (warhead == 0) return ModItems.slbm_martin_1;
            else if (warhead == 1) return ModItems.slbm_martin_2;
            else if (warhead == 2) return ModItems.slbm_martin_3;

        } else if (flightType == 2) {

            if (warhead == 0) return ModItems.slbm_pegasus_1;
            else if (warhead == 1) return ModItems.slbm_pegasus_2;
            else if (warhead == 2) return ModItems.slbm_pegasus_3;

        } else if (flightType == 3) {

            if (warhead == 0) return ModItems.slbm_spear_1;
            else if (warhead == 1) return ModItems.slbm_spear_2;
            else if (warhead == 2) return ModItems.slbm_spear_3;

        } else if (flightType == 4) {

            if (warhead == 0) return ModItems.slbm_pepper_1;
            else if (warhead == 1) return ModItems.slbm_pepper_2;
            else if (warhead == 2) return ModItems.slbm_pepper_3;
    ...
    public static void explode(Entity missile, int strength, int type) {
        ...
        if (type == 0 || type == 1) {
            ...
            worldObj
                .spawnEntityInWorld(EntityBlast.statFac(worldObj, posX, posY, posZ, strength * 2, 100, 0, 0, false));

            if (strength >= 35) worldObj.spawnEntityInWorld(EntityNukeCloudSmall.statFac(worldObj, posX, posY, posZ));
        }

        if (type == 2) {

            worldObj.spawnEntityInWorld(
                EntityNukeCloudSmall.statFac(worldObj, posX, posY, posZ)
                    .scaleMulti(2.5F));
            worldObj.spawnEntityInWorld(
                EntityNuclearBlast.statFac(
                    worldObj,
                    posX,
                    posY,
                    posZ,
                    strength,
                    MainRegistry.nukeStrength,
                    MainRegistry.nukeDist,
                    MainRegistry.nukeKill));
        }
    }
}
