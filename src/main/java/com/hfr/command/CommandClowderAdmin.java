...
public class CommandClowderAdmin extends CommandBase {
    ...
    private void cmdHelp(ICommandSender sender, String page) {
        ...
        if (p == 1) {
            sender.addChatMessage(
                new ChatComponentText(COMMAND_ADMIN + "-forcejoin <name>" + TITLE + " - Forcefully joins a faction"));
            sender.addChatMessage(
                new ChatComponentText(
                    COMMAND_ADMIN + "-forcekick <name>" + TITLE + " - Forcefully kicks a player from his faction"));
            sender.addChatMessage(
                new ChatComponentText(
                    COMMAND_ADMIN + "-forcedisband <name>" + TITLE + " - Forcefully disbands faction"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_ADMIN + "-hijack" + TITLE + " - Forcefully overrides leadership"));
            sender.addChatMessage(
                new ChatComponentText(
                    COMMAND_ADMIN + "-deletedata" + TITLE + " - Deletes all clowder data (CAUTION!!)"));
            sender.addChatMessage(
                new ChatComponentText(
                    COMMAND_ADMIN + "-setclaim <wild/safe/war> <s/c> <radius>"
                        + TITLE
                        + " - Claims chunks in a radius (square or circular)"));
            sender.addChatMessage(
                new ChatComponentText(
                    COMMAND_ADMIN + "-addprestige <name> <amount>"
                        + TITLE
                        + " - Adds prestige (neg values to subtract)"));
            sender.addChatMessage(new ChatComponentText(INFO + "/clowder help 2"));
        }

        if (p == 2) {
            sender.addChatMessage(
                new ChatComponentText(COMMAND_ADMIN + "-create <name>" + TITLE + " - Creates a faction"));
            sender.addChatMessage(
                new ChatComponentText(
                    COMMAND_ADMIN + "-disband <name>"
                        + TITLE
                        + " - Disbands a faction, name parameter for confirmation"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_ADMIN + "-rename <name>" + TITLE + " - Renames your faction"));
            // sender.addChatMessage(new ChatComponentText(COMMAND_ADMIN + "-info" + TITLE + " - Shows info on your
            // faction"));
        }
    }
    ...
    private void cmdForcejoin(ICommandSender sender, String name) {
        ...
        if (clowder == null) {
            ...
            if (tojoin != null) {
                ...
                sender
                    .addChatMessage(new ChatComponentText(INFO + "You have joined " + tojoin.getDecoratedName() + "!"));
    ...
    private void cmdForcekick(ICommandSender sender, String name) {
        ...
        if (clowder != null) {
            ...
            if (!clowder.leader.equals(kickee.getDisplayName())) {
                ...
                sender.addChatMessage(
                    new ChatComponentText(
                        INFO + "You have kicked "
                            + kickee.getDisplayName()
                            + " from the faction "
                            + clowder.getDecoratedName()
                            + "!"));
    ...
    private void cmdForcedisband(ICommandSender sender, String name) {
        ...
        if (clowder != null) {
            ...
            sender.addChatMessage(
                new ChatComponentText(INFO + "Faction " + clowder.getDecoratedName() + " has been disbanded!"));
    ...
    private void cmdDeletedata(ICommandSender sender) {
        ...
        while (Clowder.clowders.size() > 0) Clowder.clowders.get(0)
            .disbandClowder(player.worldObj);
        ...
        sender.addChatMessage(
            new ChatComponentText(
                EnumChatFormatting.OBFUSCATED + "" + EnumChatFormatting.DARK_PURPLE + "All data has been deleted!"));
    }

    private void cmdSetclaim(ICommandSender sender, String zo, String s, String r) {

        EntityPlayer player = getCommandSenderAsPlayer(sender);
        Zone zone = zo.equals("war") ? Zone.WARZONE
            : zo.equals("safe") ? Zone.SAFEZONE : zo.equals("wild") ? Zone.WILDERNESS : null;
        ...
        if (zone != null) {
            ...
            if (shape >= 0) {
                ...
                if (radius < 0 || radius > 25) {
                    ...
                    sender.addChatMessage(
                        new ChatComponentText(INFO + "Changed " + count + " chunks to " + zone.toString() + "!"));
                }

            } else {
                sender
                    .addChatMessage(new ChatComponentText(ERROR + "Invalid shape! Applicable: s (square), c (circle)"));
            }
    ...
    private void cmdAddPrestige(ICommandSender sender, String name, String amount) {
        ...
        if (clowder != null) {
            ...
            sender.addChatMessage(
                new ChatComponentText(
                    INFO + "Added " + am + " prestige to faction " + clowder.getDecoratedName() + "!"));
