...
public class CommandClowder extends CommandBase {
    ...
    public void processCommand(ICommandSender sender, String[] args) {
        ...
        if (Clowder.clowders.size() == 0) ClowderData.getData(sender.getEntityWorld());
        ...
        if (cmd.equals("help") || cmd.equals("man")) {
            ...
            if (args.length > 1) cmdHelp(sender, args[1]);
            else cmdHelp(sender, "1");
            return;
        }
        ...
        if (cmd.equals("info")) {
            ...
            if (args.length > 1) cmdInfo(sender, args[1]);
            else cmdInfo(sender, null);
        ...
        if (cmd.equals("listflags")) {
            ...
            if (args.length > 1) cmdListflags(sender, args[1]);
            else cmdListflags(sender, "1");
    ...
    private void cmdHelp(ICommandSender sender, String page) {
        ...
        if (p == 1) {
            sender
                .addChatMessage(new ChatComponentText(COMMAND + "-help {page}" + TITLE + " - The thing you just used"));
            // sender.addChatMessage(new ChatComponentText(COMMAND + "-create <name>" + TITLE + " - Creates a
            // faction"));
            // sender.addChatMessage(new ChatComponentText(COMMAND_ADMIN + "-disband <name>" + TITLE + " - Disbands a
            // faction, name parameter for confirmation"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_ADMIN + "-owner <player>" + TITLE + " - Transfers faction ownership"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND + "-comrades" + TITLE + " - Shows all members of your faction"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_LEADER + "-color <hexadecimal>" + TITLE + " - Sets the faction's color"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_LEADER + "-motd <MotD>" + TITLE + " - Sets the faction's MotD"));
            // sender.addChatMessage(new ChatComponentText(COMMAND_LEADER + "-rename <name>" + TITLE + " - Renames your
            // faction"));
        ...
        if (p == 2) {
            sender
                .addChatMessage(new ChatComponentText(COMMAND + "-info {page}" + TITLE + " - Shows info on a faction"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND + "-list" + TITLE + " - Lists all factions (page functin pending)"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND + "-apply <name>" + TITLE + " - Sends an application to a faction"));
            sender.addChatMessage(new ChatComponentText(COMMAND + "-leave" + TITLE + " - Leaves the faction"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_LEADER + "-accept <name>" + TITLE + " - Accepts a player's application"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_LEADER + "-deny <name>" + TITLE + " - Denies a player's application"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_LEADER + "-applicants" + TITLE + " - Lists applying players"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_LEADER + "-kick <player>" + TITLE + " - Removes player from faction"));
            sender.addChatMessage(new ChatComponentText(INFO + "/clowder help 3"));
        }

        if (p == 3) {
            sender.addChatMessage(
                new ChatComponentText(COMMAND_LEADER + "-flag <flag>" + TITLE + " - Changes faction flag"));
        ...
        if (p == 4) {
            sender
                .addChatMessage(new ChatComponentText(COMMAND + "-retreat" + TITLE + " - Reatreats after 10 minutes"));
            // sender.addChatMessage(new ChatComponentText(COMMAND + "-claim" + TITLE + " - Creates a new flag"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND + "-balance" + TITLE + " - Displays how much prestige the faction has"));
            sender.addChatMessage(
            ...
            sender.addChatMessage(
                new ChatComponentText(COMMAND_ADMIN + "-promote <amount>" + TITLE + " - Promotes a member to officer"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_ADMIN + "-demote <amount>" + TITLE + " - Demotes an officer to member"));
            sender.addChatMessage(
                new ChatComponentText(COMMAND_LEADER + "-nameclaim <name>" + TITLE + " - Renames the territory"));
        }
    }
    ...
    private void cmdCreate(ICommandSender sender, String name) {
        ...
        if (Clowder.getClowderFromPlayer(player) == null) {
            ...
            sender.addChatMessage(
                new ChatComponentText(ERROR + "You can not create a new faction while already being in one!"));
        }
    }
    ...
    private void cmdDisband(ICommandSender sender, String name) {
        ...
        if (clowder != null) {
            ...
            if (name.equals(clowder.name)) {
                ...
                sender.addChatMessage(
                    new ChatComponentText(
                        ERROR + "Confirmation unsuccessful. Please enter the faction name to disband the faction."));
            }
    ...
    private void cmdComrades(ICommandSender sender) {
        ...
        if (clowder != null) {
            ...
            for (String s : clowder.members.keySet()) sender.addChatMessage(new ChatComponentText(LIST + s));
    ...
    private void cmdInfo(ICommandSender sender, String name) {
        ...
        if (clowder != null) {
            ...
            sender.addChatMessage(
                new ChatComponentText(
                    LIST + "Players considered online: "
                        + clowder.getPlayersOnline()
                        + "/"
                        + clowder.members.keySet()
                            .size()));
            ...
            sender.addChatMessage(
                new ChatComponentText(
                    LIST + " -generating: "
                        + clowder.round(clowder.getPrestigeGen())
                        + " per hour (x"
                        + clowder.round((float) Math.pow(0.99, clowder.getPrestige()))
                        + ")"));
            sender.addChatMessage(
                new ChatComponentText(
                    LIST + " -requires: " + clowder.round(clowder.getPrestigeReq()) + " at all times"));
            sender.addChatMessage(
                new ChatComponentText(
                    LIST + "Color: "
                        + Integer.toHexString(clowder.color)
                            .toUpperCase()));
     ...
     * sender.addChatMessage(new ChatComponentText(LIST + "Members: " + clowder.members.size()));
     * sender.addChatMessage(new ChatComponentText(LIST + "Prestige: " + clowder.round(clowder.getPrestige())));
     * sender.addChatMessage(new ChatComponentText(LIST + "Color: " +
     * Integer.toHexString(clowder.color).toUpperCase()));
     * } else {
        ...
        if (clowder != null) {
            ...
            if (Clowder.getClowderFromName(name) == null) {
                ...
                if (clowder.getPermLevel(player.getDisplayName()) > 1) {
                    ...
                    sender.addChatMessage(
                        new ChatComponentText(ERROR + "You lack the permissions to rename this faction!"));
                }
    ...
    private void cmdList(ICommandSender sender) {
        ...
        if (Clowder.clowders.isEmpty()) {
            sender.addChatMessage(
                new ChatComponentText(TITLE + "There are no factions as of now. Use /clowder create <name>"));
    ...
    private void cmdMOTD(ICommandSender sender, String[] motd) {
        ...
        if (clowder != null) {
            ...
            if (clowder.getPermLevel(player.getDisplayName()) > 1) {
                ...
                for (int i = 1; i < motd.length; i++) stitched += motd[i] + " ";
                ...
                sender.addChatMessage(
                    new ChatComponentText(ERROR + "You lack the permissions to change this faction's MOTD!"));
            }
        ...
        if (clowder != null) {
            ...
            if (clowder.getPermLevel(player.getDisplayName()) > 2) {
                ...
                if (clowder.members.get(owner) != null) {
                    ...
                    sender
                        .addChatMessage(new ChatComponentText(INFO + "Transfered leadership to player " + owner + "!"));
                    clowder.notifyLeader(
                        player.worldObj,
                        new ChatComponentText(INFO + "You are now this faction's new leader!"));
                ...
                sender.addChatMessage(
                    new ChatComponentText(ERROR + "You can not change the color of a faction you do not own!"));
            }
    ...
    private void cmdApply(ICommandSender sender, String name) {
        ...
        if (clowder == null) {
            ...
            if (toApply != null) {
                ...
                toApply.notifyLeader(
                    player.worldObj,
                    new ChatComponentText(
                        INFO + "Player " + sender.getCommandSenderName() + " would like to join your faction!"));
            ...
            if (clowder.getPermLevel(player.getDisplayName()) > 1) {
                ...
                if (clowder.applications.contains(name)) {
                    ...
                    if (Clowder.getClowderFromName(name) == null) {
                        ...
                        sender
                            .addChatMessage(new ChatComponentText(INFO + "Added player " + name + " to your faction!"));
                        clowder.notifyPlayer(
                            player.worldObj,
                            name,
                            new ChatComponentText(
                        ...
                        sender.addChatMessage(
                            new ChatComponentText(ERROR + "This player is already in another faction!"));
                    }
                ...
                sender
                    .addChatMessage(new ChatComponentText(ERROR + "You lack the permissions to manage applications!"));
            }
    ...
    private void cmdApplicants(ICommandSender sender) {
        ...
        if (clowder != null) {
            ...
            if (clowder.getPermLevel(player.getDisplayName()) > 1) {
                ...
                if (cnt == 0) sender.addChatMessage(new ChatComponentText(LIST + "None!"));
                ...
                if (clowder.members.get(kickee) != null) {
                    ...
                    if (player.getDisplayName()
                        ...
                        clowder.notifyPlayer(
                            player.worldObj,
                            kickee,
                            new ChatComponentText(CRITICAL + "You have been kicked from your faction!"));
    ...
    private void cmdFlag(ICommandSender sender, String flag) {
        ...
        if (clowder != null) {
            ...
            if (clowder.getPermLevel(player.getDisplayName()) > 1) {
                ...
                sender.addChatMessage(
                    new ChatComponentText(ERROR + "You lack the permissions to change this faction's flag!"));
            }
    ...
    private void cmdRetreat(ICommandSender sender) {
        ...
        if (clowder != null) {
            ...
            if (!Clowder.retreating.contains(player.getDisplayName())) {
                ...
                clowder.notifyAll(
                    player.worldObj,
                    new ChatComponentText(INFO + "Player " + player.getDisplayName() + " is retreating!"));
    ...
    private void cmdHome(ICommandSender sender) {
        ...
        if (clowder != null) {
            ...
            if (owner != null
                ...
                clowder.teleports.put(
                    System.currentTimeMillis() + 10000L,
                    new ScheduledTeleport(clowder.homeX, clowder.homeY, clowder.homeZ, player.getDisplayName(), true));
            ...
            if (clowder.warps.containsKey(name)) {
                ...
                for (int i = 2; i <= 5; i++) {
                    ...
                    if (block == ModBlocks.tp_tent) {
                        ...
                        int[] pos = ((BlockDummyable) ModBlocks.tp_tent)
                            .findCore(player.worldObj, tentX, warp[1], tentZ);

                        if (pos != null) {

                            provider.loadChunk(pos[0] >> 4, pos[2] >> 4);
                            TileEntityProp tent = (TileEntityProp) player.worldObj
                                .getTileEntity(pos[0], pos[1], pos[2]);

                            if (tent.warp.equals(name) && tent.operational()) {

                                sender
                                    .addChatMessage(new ChatComponentText(INFO + "Please stand still for 10 seconds!"));
                                clowder.teleports.put(
                                    System.currentTimeMillis() + 10000L,
                                    new ScheduledTeleport(warp[0], warp[1], warp[2], player.getDisplayName(), name));
    ...
    private void cmdBalance(ICommandSender sender) {
        ...
        if (clowder != null) {
            ...
            if (clowder.getPrestige() > 0) sender.addChatMessage(
                new ChatComponentText(INFO + "Current prestige balance: " + LIST + clowder.getPrestige()));
            else sender.addChatMessage(new ChatComponentText(INFO + "It seems like you're bankrupt."));
            ...
            if (clowder.getPermLevel(player.getDisplayName()) > 2) {
                ...
                if (clowder.members.get(demotee) != null) {
                    ...
                    if (clowder.getPermLevel(demotee) == 2) {
                        ...
                        sender.addChatMessage(
                            new ChatComponentText(ERROR + "Are you seriously trying to demote the faction's leader?"));
                    }
    ...
    private void cmdNameClaim(ICommandSender sender, String name) {
        ...
        ITerritoryProvider flag = (ITerritoryProvider) player.worldObj
            .getTileEntity(meta.flagX, meta.flagY, meta.flagZ);

        if (flag == null) {
            sender.addChatMessage(
                new ChatComponentText(
                    ERROR + "The flag that is connected to this claim could not be found! Are the chunks unloaded?"));
            return;
        }
        ...
        sender.addChatMessage(
            new ChatComponentText(
                INFO + "Your claim has been renamed! It might take a few moments for all chunks to assume the new name."));
        ClowderData.getData(player.worldObj)
            .markDirty();
    }
