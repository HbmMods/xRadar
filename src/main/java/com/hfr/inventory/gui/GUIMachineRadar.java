    ...
    public void drawScreen(int mouseX, int mouseY, float f) {
        ...
        if (!diFurnace.nearbyMissiles.isEmpty()) {
            for (RadarEntry m : diFurnace.nearbyMissiles) {
                int x = guiLeft
                    + (int) ((m.posX - diFurnace.xCoord) / ((double) MainRegistry.radarRange * 2 + 1) * (200D - 8D))
                    + 108;
                int z = guiTop
                    + (int) ((m.posZ - diFurnace.zCoord) / ((double) MainRegistry.radarRange * 2 + 1) * (200D - 8D))
                    + 117;
    ...
    protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_) {
        ...
        if (!diFurnace.nearbyMissiles.isEmpty()) {
            for (RadarEntry m : diFurnace.nearbyMissiles) {
                ...
                int x = (int) ((m.posX - diFurnace.xCoord) / ((double) MainRegistry.radarRange * 2 + 1) * (200D - 8D))
                    - 4;
                int z = (int) ((m.posZ - diFurnace.zCoord) / ((double) MainRegistry.radarRange * 2 + 1) * (200D - 8D))
                    - 4;
                int t = 1;
