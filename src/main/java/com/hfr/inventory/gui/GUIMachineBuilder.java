...
public class GUIMachineBuilder extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int i, int j) {
        ...
        if (TileEntityMachineBuilder.offers.size() == 0) return;

        if (getSchematic(index) == null) return;
        ...
        this.fontRendererObj
            .drawString(name, this.xSize / 2 - this.fontRendererObj.getStringWidth(name) / 2, 21, 4210752);
        this.fontRendererObj
            .drawString(val, this.xSize / 2 - this.fontRendererObj.getStringWidth(val) / 2, 34, 4210752);
    ...
    protected void mouseClicked(int x, int y, int i) {
        ...
        if (guiLeft + 61 <= x && guiLeft + 61 + 18 > x && guiTop + 52 < y && guiTop + 52 + 18 >= y) {
            ...
            PacketDispatcher.wrapper.sendToServer(
                new AuxButtonPacket(diFurnace.xCoord, diFurnace.yCoord, diFurnace.zCoord, getIndex(index), 0));
        }
        ...
        if (guiLeft + 97 <= x && guiLeft + 97 + 18 > x && guiTop + 52 < y && guiTop + 52 + 18 >= y) {
            ...
            mc.getSoundHandler()
                .playSound(PositionedSoundRecord.func_147674_a(new ResourceLocation("gui.button.press"), 1.0F));
            PacketDispatcher.wrapper.sendToServer(
                new AuxButtonPacket(diFurnace.xCoord, diFurnace.yCoord, diFurnace.zCoord, getIndex(index), 1));
        }
         ...
         * diFurnace.zCoord, getIndex(index), 2));
         * }
         */
    }
