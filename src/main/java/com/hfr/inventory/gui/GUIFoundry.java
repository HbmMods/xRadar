...
public class GUIFoundry extends GuiContainer {
    ...
    public static ResourceLocation texture = new ResourceLocation(
        RefStrings.MODID + ":textures/gui/nuMachines/gui_caster.png");
    ...
    protected void mouseClicked(int x, int y, int i) {
        ...
        if (guiLeft + 108 <= x && guiLeft + 108 + 6 > x && guiTop + 21 < y && guiTop + 21 + 8 >= y) {
            PacketDispatcher.wrapper
                .sendToServer(new AuxButtonPacket(foundry.xCoord, foundry.yCoord, foundry.zCoord, 0, 0));
            mc.getSoundHandler()
        ...
        if (guiLeft + 134 <= x && guiLeft + 134 + 6 > x && guiTop + 21 < y && guiTop + 21 + 8 >= y) {
            PacketDispatcher.wrapper
                .sendToServer(new AuxButtonPacket(foundry.xCoord, foundry.yCoord, foundry.zCoord, 0, 1));
            mc.getSoundHandler()
                .playSound(PositionedSoundRecord.func_147674_a(new ResourceLocation("gui.button.press"), 1.0F));
        }
    }
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        String name = this.foundry.hasCustomInventoryName() ? this.foundry.getInventoryName()
            : I18n.format(this.foundry.getInventoryName());
        ...
        if (guiLeft + 115 <= x && guiLeft + 115 + 18 > x && guiTop + 16 < y && guiTop + 16 + 18 >= y) {
            this.func_146283_a(
                Arrays.asList(
                    new String[] { EnumChatFormatting.YELLOW + I18n.format(
                        foundry.options.get(foundry.index)
                            .getUnlocalizedName() + ".name"),
                        "Costs " + foundry.recipes.get(
                            foundry.options.get(foundry.index)
                                .getUnlocalizedName()) }),
                x - guiLeft,
                y - guiTop);
        ...
        if (guiLeft + 26 <= x && guiLeft + 52 + 18 > x && guiTop + 37 < y && guiTop + 37 + 12 >= y) {
            this.func_146283_a(Arrays.asList(new String[] { foundry.steel + " Ingots" }), x - guiLeft, y - guiTop);
        }
