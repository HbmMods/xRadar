...
public class GUIScreenDesignator extends GuiScreen {
    ...
    private void updateButtons() {
        ...
        buttons
            .add(new FolderButton(guiLeft + 133, guiTop + 44 + 72, 10, 2, 1, 0, "Set coord to current Z position..."));
    }

    protected void mouseClicked(int i, int j, int k) {
        try {
            for (FolderButton b : buttons) if (b.isMouseOnButton(i, j)) b.executeAction();
        } catch (Exception ex) {}
    }
    ...
    protected void drawGuiContainerForegroundLayer(int i, int j) {
        ...
        // guiLeft + this.xSize / 2 - this.fontRendererObj.getStringWidth(I18n.format((currentPage + 1) + "/" +
        // (getPageCount() + 1))) / 2, guiTop + 10, 4210752);

        for (FolderButton b : buttons) if (b.isMouseOnButton(i, j)) b.drawString(i, j);

        String x = String.valueOf(shownX);
        String z = String.valueOf(shownZ);
        this.fontRendererObj.drawString(
            "X: " + x,
            guiLeft + this.xSize / 2 - this.fontRendererObj.getStringWidth("X: " + x) / 2,
            guiTop + 50,
            4210752);
        this.fontRendererObj.drawString(
            "Z: " + z,
            guiLeft + this.xSize / 2 - this.fontRendererObj.getStringWidth("Z: " + z) / 2,
            guiTop + 50 + 18 * 4,
            4210752);
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
        ...
        for (FolderButton b : buttons) b.drawButton(b.isMouseOnButton(i, j));
    }
    ...
    public void updateScreen() {
        if (player.getHeldItem() == null || player.getHeldItem()
            .getItem() != ModItems.designator_manual) player.closeScreen();
    }
    ...
    class FolderButton {
        ...
        public void executeAction() {
            ...
            if (operator == 2) {
                if (reference == 0) shownX = (int) Math.round(player.posX);
                else shownZ = (int) Math.round(player.posZ);
                return;
            }
