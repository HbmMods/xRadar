...
public class GUIMachineNet extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        if (!diFurnace.operational()) {
            if (guiLeft + 99 <= x && guiLeft + 99 + 10 > x && guiTop + 38 < y && guiTop + 38 + 10 >= y) {
                this.func_146283_a(
                    Arrays.asList(
                        new String[] { "-Net needs to be placed on top of water",
                            "-Net requires sky access and at least 3 blocks",
                            " of water underneath every block covered" }),
                    x - guiLeft,
                    y - guiTop);
            }
        }
    ...
    protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_) {
        ...
        if (diFurnace.slots[4] != null) drawTexturedModalRect(guiLeft + 121, guiTop + 38, 176, 0, 10, 10);
    }
}
