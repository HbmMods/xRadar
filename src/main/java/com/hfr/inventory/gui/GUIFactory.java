...
public class GUIFactory extends GuiContainer {
    ...
    public static ResourceLocation texture = new ResourceLocation(
        RefStrings.MODID + ":textures/gui/gui_factory_new.png");
    ...
    protected void drawGuiContainerForegroundLayer(int x, int y) {
        ...
        if (!diFurnace.operational()) {
            if (guiLeft + 99 <= x && guiLeft + 99 + 10 > x && guiTop + 38 < y && guiTop + 38 + 10 >= y) {
                this.func_146283_a(
                    Arrays.asList(
                        new String[] { "-Factory needs to be placed on top of foundation",
                            "-Factory requires sky access", "-Factory requires electricity" }),
                    x - guiLeft,
                    y - guiTop);
            }
        }
