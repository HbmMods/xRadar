...
public class GUIHydro extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int i, int j) {
        ...
        this.fontRendererObj
            .drawString("Output: " + (diFurnace.getRate(walls, turbines) / 500D) + " TE/s", 16, 44, 4210752);

        if (walls > 0 && turbines > 0) this.fontRendererObj.drawString(
            Math.round(1D / (diFurnace.getRate(walls, turbines) / 500D) * 10D) / 10D + " s/TE",
            54,
            54,
            4210752);
    }
