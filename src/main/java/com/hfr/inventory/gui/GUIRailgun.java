...
public class GUIRailgun extends GuiContainer {
    ...
    protected void drawGuiContainerForegroundLayer(int i, int j) {
        String name = this.railgun.hasCustomInventoryName() ? this.railgun.getInventoryName()
            : I18n.format(this.railgun.getInventoryName());
        ...
        if (!isTargeting() && !isFiring()) {
            if (guiLeft + 79 <= x && guiLeft + 79 + 18 > x && guiTop + 16 < y && guiTop + 16 + 18 >= y) {
                PacketDispatcher.wrapper
                    .sendToServer(new AuxButtonPacket(railgun.xCoord, railgun.yCoord, railgun.zCoord, 0, 0));
            }

            if (guiLeft + 106 <= x && guiLeft + 106 + 18 > x && guiTop + 16 < y && guiTop + 16 + 18 >= y) {
                PacketDispatcher.wrapper
                    .sendToServer(new AuxButtonPacket(railgun.xCoord, railgun.yCoord, railgun.zCoord, 0, 1));
            }
        }
    }
    ...
    protected void drawGuiContainerBackgroundLayer(float p_146976_1_, int p_146976_2_, int p_146976_3_) {
        ...
        if (isTargeting()) drawTexturedModalRect(guiLeft + 79, guiTop + 16, 176, 0, 18, 18);

        if (isFiring()) drawTexturedModalRect(guiLeft + 106, guiTop + 16, 194, 0, 18, 18);
    }
