    ...
    public ItemStack transferStackInSlot(EntityPlayer p_82846_1_, int par2) {
        ...
        if (var4 != null && var4.getHasStack()) {
            ...
            if (par2 <= 10) {
                ...
                ItemStack stack = var4.getStack();
                var3 = stack.copy();

                // if empty canister, try to put in canister slots
                if (var3.getItem() == ModItems.canister_empty) {
                    if (!this.mergeItemStack(var5, 3, 4, false)) if (!this.mergeItemStack(var5, 5, 6, false))
                        if (!this.mergeItemStack(var5, 7, 8, false)) return null;

                    // if full canister, try to put in input slot
                } else if (var3.getItem() == ModItems.canister_oil) {
                    if (!this.mergeItemStack(var5, 1, 2, false)) return null;

                    // if empty tank, try to put in tank slot
                } else if (var3.getItem() == ModItems.gas_empty) {
                    if (!this.mergeItemStack(var5, 9, 10, false)) return null;

                    // otherwise, put in battery slot
