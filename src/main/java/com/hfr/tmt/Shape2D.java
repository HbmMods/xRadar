...
public class Shape2D {
    ...
    public Shape3D extrude(float x, float y, float z, float rotX, float rotY, float rotZ, float depth, int u, int v,
        float textureWidth, float textureHeight, int shapeTextureWidth, int shapeTextureHeight, int sideTextureWidth,
        int sideTextureHeight, float[] faceLengths) {
        ...
        if (faceLengths != null && faceLengths.length < coords.size()) faceLengths = null;
        ...
        for (int idx = 0; idx < coords.size(); idx++) {
            ...
            verts[idx + coords.size()] = new PositionTransformVertex(
                ...
                z + (float) vecCoord.zCoord - (float) extrudeVector.zCoord,
                texU2,
                texV);
            ...
            if (faceLengths != null) totalLength += faceLengths[idx];
            else totalLength += Math.sqrt(
        ...
        for (int idx = 0; idx < coords.size(); idx++) {
            ...
            float currentLength = (float) Math.sqrt(
                Math.pow(curCoord.xCoord - nextCoord.xCoord, 2) + Math.pow(curCoord.yCoord - nextCoord.yCoord, 2));
            if (faceLengths != null) currentLength = faceLengths[faceLengths.length - idx - 1];
