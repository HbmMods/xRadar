...
public class EntityAIBreaking extends EntityAIBase {
    ...
    public boolean shouldExecute() {
        ...
        if (target != null && entityDigger.getNavigator()
            .noPath()
    ...
    public boolean continueExecuting() {
        // return target != null && entityDigger != null && target.isEntityAlive() && entityDigger.isEntityAlive() &&
        // markedLoc != null && entityDigger.getNavigator().noPath() && entityDigger.getDistanceToEntity(target) > 1D &&
        // (target.onGround || !entityDigger.canEntityBeSeen(target));
    ...
    public void updateTask() {
        ...
        if (markedLoc == null || markedLoc.length != 3
            || entityDigger.worldObj.getBlock(markedLoc[0], markedLoc[1], markedLoc[2]) == Blocks.air) {
            digTick = 0;
            return;
        }
