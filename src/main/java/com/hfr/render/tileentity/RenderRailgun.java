...
public class RenderRailgun extends TileEntitySpecialRenderer {
    ...
    public void renderTileEntityAt(TileEntity tile, double x, double y, double z, float f) {
        ...
        if (System.currentTimeMillis() < gun.startTime + gun.cooldownDurationMillis) {
            float interpolation = (float) (System.currentTimeMillis() - gun.startTime)
                / (float) gun.cooldownDurationMillis
                * 100F;
        ...
        int max = 5;
        int count = max - (int) (((gun.fireTime + gun.cooldownDurationMillis) - System.currentTimeMillis()) * max
            / gun.cooldownDurationMillis);
        ...
        if (System.currentTimeMillis() < gun.fireTime + gun.cooldownDurationMillis) {
            ...
            GL11.glRotatef(180, 0F, 1F, 0F);
            for (int i = 0; i < count; i++) RenderSparks.renderSpark(
