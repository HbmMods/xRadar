...
public abstract class EntityMissileBaseCruise extends Entity implements IChunkLoader, IEmpable {
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) this.dataWatcher.updateObject(10, y);
        else y = this.dataWatcher.getWatchableObjectInt(10);
        ...
        if (this.worldObj.isRemote) MainRegistry.proxy.howDoIUseTheZOMG(worldObj, posX, posY, posZ, 5);
        ...
        if (!worldObj.isRemote) {
            ...
            if (mov.lengthVector() < 0.1) this.setDead();
        }
    }
