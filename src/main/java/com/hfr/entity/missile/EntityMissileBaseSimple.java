...
public abstract class EntityMissileBaseSimple extends Entity implements IChunkLoader, IEmpable {
    ...
    public void onUpdate() {
        ...
        if (mode == 0) {
            ...
            if (dataWatcher.getWatchableObjectInt(10) <= 0) mode = 2;

        } else {

            // to pull deflected missiles back to earth
            if (Vec3.createVectorHelper(motionX, motionY, motionZ)
                .lengthVector() < 5) motionY -= 0.1;
        }
        ...
        if (mode != 1) this.setLocationAndAngles(posX + this.motionX, posY + this.motionY, posZ + this.motionZ, 0, 0);
    ...
    public void loadNeighboringChunks(int newChunkX, int newChunkZ) {
        if (!worldObj.isRemote && loaderTicket != null) {
            ...
            for (ChunkCoordIntPair chunk : loadedChunks) {
                if (!loaderTicket.getChunkList()
                    .contains(chunk)) ForgeChunkManager.forceChunk(loaderTicket, chunk);
            }
        }
    }
