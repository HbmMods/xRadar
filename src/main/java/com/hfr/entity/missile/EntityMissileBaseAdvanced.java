...
public abstract class EntityMissileBaseAdvanced extends Entity implements IChunkLoader, IEmpable {
    ...
    public void onUpdate() {
        ...
        if (this.ticksExisted > 40) velocity = 3;
        else if (this.ticksExisted > 20) velocity = 2;
        ...
        for (int i = 0; i < velocity; i++) {
            ...
            if (this.worldObj.isRemote && trailing()) {
                // PacketDispatcher.wrapper.sendToAllAround(new ParticleControlPacket(posX, posY, posZ, 0), new
                // TargetPoint(this.dimension, posX, posY, posZ, 500));
                // Minecraft.getMinecraft().effectRenderer.addEffect(new EntitySmokeFX(this.worldObj, this.posX,
                // this.posY, this.posZ, 0.0, 0.0, 0.0));
                // this.worldObj.spawnParticle("largeexplode", this.posX, this.posY, this.posZ, 1.0D, 0.0D, 0.0D);

                // EntityLargeExplodeFX effect = new EntityLargeExplodeFX(Minecraft.getMinecraft().getTextureManager(),
                // worldObj, posX, posY, posZ, 0.0, 0.0, 0.0);
        ...
        if (!worldObj.isRemote) {
            ...
            if (movement.lengthVector() < 0.1) this.setDead();
        }
    }
