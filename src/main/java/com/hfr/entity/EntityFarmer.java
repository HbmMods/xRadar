...
public class EntityFarmer extends EntityLiving {
    ...
    public void onUpdate() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (posY < 40 || !this.worldObj.canBlockSeeTheSky(
            ...
            for (int j = (int) (posZ - range); j <= (int) (posZ + range); j++) {
                ...
                if ((b1 == Blocks.dirt || b1 == Blocks.grass) && !b.isNormalCube() && rand.nextInt(5) == 0) {
                    worldObj.playSoundEffect(
                        ...
                        posY,
                        posZ,
                        b1.stepSound.getStepResourcePath(),
                        (b1.stepSound.getVolume() + 1.0F) / 2.0F,
                        b1.stepSound.getPitch() * 0.8F);
                    worldObj.setBlock(i, (int) posY - 1, j, Blocks.farmland);
                }

                // FARM SUGAR
                if (getType() == 1 && (b1 == Blocks.dirt || b1 == Blocks.grass || b1 == Blocks.sand)
                    && b == Blocks.reeds) {
    ...
    private void processCrop(Block b) {
        ...
        if (b == Blocks.wheat) wheat += 2;

        if (b == Blocks.potatoes) potato += (3 * (type == 0 ? 3 : 1));

        if (b == Blocks.carrots) carrot += 3;

        if (b == ModBlocks.rice) rice += 2 + (type == 1 ? 2 : 0);

        if (b == Blocks.reeds) sugar += 1;

        if (b == Blocks.pumpkin) pumpkin += 1;

        if (b == Blocks.melon_block) melon += 1;

        if (wheat > 64 * 3) wheat = 64 * 3;
        if (potato > 64 * 3) potato = 64 * 3;
        if (carrot > 64 * 3) carrot = 64 * 3;
        if (rice > 64 * 3) rice = 64 * 3;

        if (sugar > 64 * 2) sugar = 64 * 2;
        if (pumpkin > 64 * 2) pumpkin = 64 * 2;
        if (melon > 64 * 2) melon = 64 * 2;
    }

    protected void dropFewItems(boolean p_70628_1_, int p_70628_2_) {

        if (wheat > 0) this.dropItem(Items.wheat, wheat);
        if (potato > 0) this.dropItem(Items.potato, potato);
        if (carrot > 0) this.dropItem(Items.carrot, carrot);
        if (rice > 0) this.dropItem(ModItems.rice, rice);
        if (sugar > 0) this.dropItem(Items.reeds, sugar);
        if (pumpkin > 0) this.dropItem(Item.getItemFromBlock(Blocks.pumpkin), pumpkin);
        if (melon > 0) this.dropItem(Item.getItemFromBlock(Blocks.melon_block), melon);
    ...
    public boolean interact(EntityPlayer player) {
        if (player.getHeldItem() != null && player.getHeldItem()
            .getItem() instanceof ItemFood && fuel < 60 * 60 * 20) {

            fuel += ((ItemFood) player.getHeldItem()
                .getItem()).func_150905_g(player.getHeldItem()) * 1200;
