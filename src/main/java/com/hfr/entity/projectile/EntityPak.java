...
public class EntityPak extends Entity {
        ...
        if (movingobjectposition != null) {
            ...
            if (!this.worldObj.isRemote) {
                ...
                List<Entity> entities = worldObj.getEntitiesWithinAABB(
                    Entity.class,
                    AxisAlignedBB
                        .getBoundingBox(posX - rad, posY - rad, posZ - rad, posX + rad, posY + rad, posZ + rad));
                ...
                // ExplosionController.registerExplosion(new ExplosionNukeRay(worldObj, (int)posX, (int)posY, (int)posZ,
                // 100, 50));

                PacketDispatcher.wrapper.sendToAllAround(
                    new ParticleControlPacket(posX - motionX * 2, posY - motionY * 2, posZ - motionZ * 2, 7),
                    new TargetPoint(this.dimension, posX, posY, posZ, 250));
    ...
    public static EntityPak statFac(World world, EntityPlayer player) {
        ...
        flare.yOffset = 0.0F;
        float f = 1F;
        flare.motionX = (double) (-MathHelper.sin(flare.rotationYaw / 180.0F * (float) Math.PI)
            * MathHelper.cos(flare.rotationPitch / 180.0F * (float) Math.PI)
            * f);
        flare.motionZ = (double) (MathHelper.cos(flare.rotationYaw / 180.0F * (float) Math.PI)
            * MathHelper.cos(flare.rotationPitch / 180.0F * (float) Math.PI)
            * f);
    ...
    public void rotation() {
        ...
        for (this.rotationPitch = (float) (Math.atan2(this.motionY, f2) * 180.0D / Math.PI) - 90; this.rotationPitch
