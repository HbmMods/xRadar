...
public class EntityEMP extends Entity {
    ...
    private void add(int x, int y, int z) {
        ...
        if (MainRegistry.empSpecial && te instanceof IEnergyProvider || te instanceof IEnergyReceiver
    ...
    private void emp(int x, int y, int z) {
        ...
        if (te != null && te instanceof IEnergyHandler) {
            ...
            for (int i = 0; i < 10; i++) {
                ((IEnergyHandler) te)
                    .extractEnergy(ForgeDirection.UP, ((IEnergyHandler) te).getEnergyStored(ForgeDirection.UP), false);
                ((IEnergyHandler) te).extractEnergy(
                    ForgeDirection.DOWN,
                    ((IEnergyHandler) te).getEnergyStored(ForgeDirection.DOWN),
                    false);
                ((IEnergyHandler) te).extractEnergy(
                    ForgeDirection.NORTH,
                    ((IEnergyHandler) te).getEnergyStored(ForgeDirection.NORTH),
                    false);
                ((IEnergyHandler) te).extractEnergy(
                    ForgeDirection.SOUTH,
                    ((IEnergyHandler) te).getEnergyStored(ForgeDirection.SOUTH),
                    false);
                ((IEnergyHandler) te).extractEnergy(
                    ForgeDirection.EAST,
                    ((IEnergyHandler) te).getEnergyStored(ForgeDirection.EAST),
                    false);
                ((IEnergyHandler) te).extractEnergy(
                    ForgeDirection.WEST,
                    ((IEnergyHandler) te).getEnergyStored(ForgeDirection.WEST),
                    false);
            }
            flag = true;
        }
        ...
        if (flag && rand.nextInt(MainRegistry.empParticle) == 0) {
            ...
            PacketDispatcher.wrapper.sendToAllAround(
                new ParticleBurstPacket(x, y, z, Block.getIdFromBlock(Blocks.stained_glass), 3),
                new TargetPoint(this.dimension, x, y, z, 10));
