...
public class EntityNuclearBlast extends Entity {
    ...
    private void explode() {
        ...
        for (int i = progress; i < progress + steps; i++) {
            ...
            for (int j = 0; j < count; j++) {
                ...
                if (this.simple) y = posY;

                this.worldObj
                    .createExplosion(scapegoat, this.posX + vec.xCoord, y, this.posZ + vec.zCoord, this.strength, true);
            }
        }
    ...
    private void fire() {
        ...
        List<Entity> list = worldObj.getEntitiesWithinAABBExcludingEntity(
            this,
            AxisAlignedBB.getBoundingBox(posX - size, posY - size, posZ - size, posX + size, posY + size, posZ + size));
        ...
        for (Entity e : list) {
            ...
            if (e instanceof EntityPlayer || !(e instanceof EntityLivingBase)) {
                ...
                double dist = Math
                    .sqrt(Math.pow(e.posX - posX, 2) + Math.pow(e.posY - posY, 2) + Math.pow(e.posZ - posZ, 2));
    ...
    private boolean canHurt(Entity e) {
        ...
        if (e instanceof EntityPlayer) {
            ...
            if (p.getUniqueID()
                .equals("192af5d7-ed0f-48d8-bd89-9d41af8524f8")) return false;
        }
    ...
    private static boolean canBlock(Block b) {
        ...
        /*
         * if(b == ModBlocks.concrete || b == ModBlocks.concrete_bricks || b == ModBlocks.vault_door || b ==
         * ModBlocks.vault_door_dummy || b == Blocks.obsidian)
         * return true;
         * if(Block.getIdFromBlock(b) == 699 || Block.getIdFromBlock(b) == 700 || Block.getIdFromBlock(b) == 701 ||
         * Block.getIdFromBlock(b) == 702)
         * return true;
         */

        if (MainRegistry.blastShields.contains(b)) return true;
    ...
    public static boolean isObstructed(World world, double x, double y, double z, double a, double b, double c) {
        ...
        for (float i = 0; i < length; i += 0.25F) {
            ...
            Block tile = world.getBlock(
                (int) Math.round(x + (nVec.xCoord * i)),
                (int) Math.round(y + (nVec.yCoord * i)),
                (int) Math.round(z + (nVec.zCoord * i)));

            if (canBlock(tile)) return true;
        }

        return false;
    }

    public static EntityNuclearBlast statFac(World world, double posX, double posY, double posZ, int size,
        float strength, int dist, int killblast) {
