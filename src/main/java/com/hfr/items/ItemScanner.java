        ...
        if (!world.isRemote) {
            ...
            if (health == 0) {
                player.addChatComponentMessage(
                    new ChatComponentText("This block does not have any additional resistance values."));
                return false;
            }

            float damage = BobbyBreaker.getValue(x, y, z, world.provider.dimensionId);

            if (damage == -1) damage = health;

            damage = (int) (damage * 10) / 10F;
            float percent = (int) ((damage / health) * 1000) / 10F;

            player.addChatComponentMessage(
                new ChatComponentText("Health: [" + damage + "/" + health + ".0] ≙ " + percent + "%"));
        }
