...
public class BlockSeal extends Block {
    ...
    public IIcon getIcon(int side, int metadata) {
        ...
        return side == 1 ? this.iconTop
            : (side == 0 ? this.iconTop
                : (metadata == 0 && side == 3 ? this.blockIcon : (side == metadata ? this.blockIcon : this.iconTop)));
    }
    ...
    public static int getFrameSize(World world, int x, int y, int z) {
        ...
        for (int size = 1; size < max; size++) {
            ...
            int xOff = 0;
            int zOff = 0;
            if (world.getBlockMetadata(x, y, z) % 4 == 2) zOff += size;
            if (world.getBlockMetadata(x, y, z) % 4 == 3) zOff -= size;
            if (world.getBlockMetadata(x, y, z) % 4 == 0) xOff += size;
            if (world.getBlockMetadata(x, y, z) % 4 == 1) xOff -= size;

            for (int X = x - size; X <= x + size; X++) {
                if (world.getBlock(X + xOff, y, z + size + zOff) != ModBlocks.seal_frame
                    && world.getBlock(X + xOff, y, z + size + zOff) != ModBlocks.seal_controller) valid = false;
            }
            for (int X = x - size; X <= x + size; X++) {
                if (world.getBlock(X + xOff, y, z - size + zOff) != ModBlocks.seal_frame
                    && world.getBlock(X + xOff, y, z - size + zOff) != ModBlocks.seal_controller) valid = false;
            ...
            for (int Z = z - size; Z <= z + size; Z++) {
                if (world.getBlock(x - size + xOff, y, Z + zOff) != ModBlocks.seal_frame
                    && world.getBlock(x - size + xOff, y, Z + zOff) != ModBlocks.seal_controller) valid = false;
            }
            for (int Z = z - size; Z <= z + size; Z++) {
                if (world.getBlock(x + size + xOff, y, Z + zOff) != ModBlocks.seal_frame
                    && world.getBlock(x + size + xOff, y, Z + zOff) != ModBlocks.seal_controller) valid = false;
        ...
        for (int X = x - size + 1; X <= x + size - 1; X++) {
            for (int Z = z - size + 1; Z <= z + size - 1; Z++) {
                if (world.getBlock(X + xOff, y, Z + zOff) == Blocks.air && !world.isRemote) {
                    ...
                    if (te != null && te instanceof TileEntityHatch) ((TileEntityHatch) te).setControllerPos(x, y, z);
    ...
    public void onNeighborBlockChange(World world, int x, int y, int z, Block block) {
        ...
        if (world.isBlockIndirectlyGettingPowered(x, y, z)) {
            ...
            if (meta < 4) {
                ...
                if (i != 0) if (BlockSeal.isSealClosed(world, x, y, z, i)) BlockSeal.openSeal(world, x, y, z, i);
                else BlockSeal.closeSeal(world, x, y, z, i);
            }
        } else {
            int meta = world.getBlockMetadata(x, y, z);
            if (meta >= 4) {
