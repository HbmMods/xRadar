...
public class Flag extends BlockContainer {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase player, ItemStack itemStack) {
        ...
        if (player instanceof EntityPlayer && !world.isRemote) {
            ...
            if (clowder != null && flag.canSeeSky()) {
                ...
                ((EntityPlayer) player).addChatMessage(
                    new ChatComponentText(EnumChatFormatting.RED + "-The flag lacking a solid 5x5 block foundation"));
                ((EntityPlayer) player).addChatMessage(
                    new ChatComponentText(EnumChatFormatting.RED + "-The flag's foundation not having sky access"));
                ((EntityPlayer) player)
                    .addChatMessage(new ChatComponentText(EnumChatFormatting.RED + "-You not being in any faction"));
                ((EntityPlayer) player).addChatMessage(
                    new ChatComponentText(EnumChatFormatting.RED + "-The flag being below Y:45 or above Y:100"));
            }
    ...
    public void breakBlock(World world, int x, int y, int z, Block b, int i) {
        ...
        if (owner != null && owner.zone == Zone.FACTION) {
            ...
            if (flag.owner != null) flag.setOwner(null);
        }
