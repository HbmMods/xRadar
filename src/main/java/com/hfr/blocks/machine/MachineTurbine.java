...
public class MachineTurbine extends BlockDummyable {
    ...
    public TileEntity createNewTileEntity(World world, int meta) {
        ...
        if (meta >= 12) return new TileEntityMachineTurbine();

        if (meta >= 6) return new TileEntityComboProxy();
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX,
        ...
        if (world.isRemote) {
            ...
            FMLNetworkHandler
                .openGui(player, MainRegistry.instance, ModBlocks.guiID_turbine, world, pos[0], pos[1], pos[2]);
            return true;
        } else {
            return true;
        }
