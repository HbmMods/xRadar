        ...
        public IMessage onMessage(AuxButtonPacket m, MessageContext ctx) {
            ...
            try {
                    ...
                    if (m.id == 0) {
                        if (gun.setAngles(p.isPotionActive(HFRPotion.emp))) {
                            ...
                            PacketDispatcher.wrapper
                                .sendToAll(new RailgunCallbackPacket(m.x, m.y, m.z, gun.pitch, gun.yaw));
                        } else {
                ...
                if (te instanceof TileEntityMachineMarket) {
                    ...
                    if (m.id == 0) market.buyStock(data.getByInt(m.value), p.getDisplayName());
                    if (m.id == 1) market.sellStock(data.getByInt(m.value), p.getDisplayName());
                }
                ...
                if (te instanceof TileEntityMachineBuilder) {
                    ...
                    if (m.id == 0) {
                        ...
                        if (builder.preview == null) {
                            ...
                            PacketDispatcher.wrapper
                                .sendToAll(new SchematicPreviewPacket(m.x, m.y, m.z, MainRegistry.schems.get(m.value)));
                        } else {
                ...
                if (te instanceof TileEntityFlag) {
                    ...
                    if (flag.isClaimed) {
                        ...
                        if (m.id == 0) {
                            ...
                            if (flag.mode != 1) flag.setMode(1);
                            else flag.setMode(0);
                        }

                        if (m.id == 1) {

                            if (flag.mode != 2) flag.setMode(2);
                            else flag.setMode(0);
                        }

                        if (m.id == 2) {

                            if (flag.mode != 3) flag.setMode(3);
                            else flag.setMode(0);
                        }

                        if (flag.mode > 0) flag.generateClaim();
                    }
                     ...
                     * "This claim is protected by surrounding claims."));
                     * p.addChatComponentMessage(new ChatComponentText(EnumChatFormatting.RED +
                     * "Capture outer claims before proceeding to this one!"));
                     * }
                     * }
                     * }
                     */
                ...
                if (te instanceof TileEntityFoundry) {
                    ...
                    if (m.id == 0) foundry.decrement();
                    else if (m.id == 1) foundry.increment();
                }
                ...
                if (te instanceof TileEntityMarket) {
                    ...
                    if (m.value < 0 || m.value >= data.offers.get(market.name)
                        .size()) {
                    ...
                    ItemStack[] offer = data.offers.get(market.name)
                        .get(m.value);
                    ...
                    if (offer != null) {
                        ...
                        for (int i = 1; i < 4; i++) {
                            ...
                            if (offer[i] != null) {
                                ...
                                if (count < offer[i].stackSize) flag = false;
                            }
                        }
                        ...
                        if (flag) {
                            ...
                            p.addChatComponentMessage(
                                new ChatComponentText(EnumChatFormatting.RED + "You lack required items."));
                        }
                    } else {
