    ...
    public static class Handler implements IMessageHandler<ItemDesignatorPacket, IMessage> {
        ...
        public IMessage onMessage(ItemDesignatorPacket m, MessageContext ctx) {
            ...
            if (stack != null && stack.getItem() == ModItems.designator_manual) {
                ...
                if (m.operator == 0) result += m.value;
                if (m.operator == 1) result -= m.value;
                if (m.operator == 2) {
                    if (m.reference == 0) stack.stackTagCompound.setInteger("xCoord", (int) Math.round(p.posX));
                    else stack.stackTagCompound.setInteger("zCoord", (int) Math.round(p.posZ));
                    return null;
                }

                if (m.reference == 0) stack.stackTagCompound.setInteger("xCoord", x + result);
                else stack.stackTagCompound.setInteger("zCoord", z + result);
            }
