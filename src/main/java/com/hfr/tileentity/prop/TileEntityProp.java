...
public class TileEntityProp extends TileEntity {
        ...
        if (!worldObj.isRemote) {
            ...
            if (!warp.isEmpty()) {
                ...
                if (!operational() || o == null || o.zone != Zone.FACTION || !o.owner.warps.containsKey(warp)) {
                    ...
                    if (owner != null) owner.warps.remove(warp);
                ...
                if (this.operational()) {
                    ...
                    if (owner != o.owner) {
                        ...
                        if (owner != null) {
                            ...
                            // doesn't need a call of the save method because it will always save in the statements
                            // below
    ...
    public boolean operational() {
        ...
        int[] dim = MultiblockHandler.rotate(
            dummy.getDimensions(),
        ...
        for (int i = -w; i <= e; i++)
            for (int j = -n; j <= s; j++) if (worldObj.getBlock(xCoord + i, yCoord + 4, zCoord + j)
                .getMaterial() != Material.air && !(i == 0 && j == 0)
                || !worldObj.canBlockSeeTheSky(xCoord + i, yCoord + 4, zCoord + j)) return false;

        for (int x = -w; x <= e; x++) for (int z = -n; z <= s; z++)
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        if (!warp.isEmpty()) nbt.setString("warp", warp);
    }
