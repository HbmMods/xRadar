...
public class TileEntityOfficerChest extends TileEntity implements IInventory {
    ...
    public boolean isUseableByPlayer(EntityPlayer player) {
        ...
        if (c == null) {
            player
                .addChatComponentMessage(new ChatComponentText(EnumChatFormatting.RED + "You are not in any faction!"));
            return false;
        }

        if (c.getPermLevel(player.getDisplayName()) < 2) {
            player.addChatComponentMessage(
                new ChatComponentText(EnumChatFormatting.RED + "You lack permission to open this chest!"));
            return false;
        }
