...
public class TileEntityFlag extends TileEntityMachineBase implements ITerritoryProvider {
        ...
        if (!worldObj.isRemote) {
            ...
            if (!Clowder.clowders.contains(owner) && owner != null) {
                MainRegistry.logger.info(
                    "Deleting clowder from flag " + xCoord
                        + " "
                        + yCoord
                        + " "
                        + zCoord
                        + " due to clowder not being in the clowder list! (disband?)");
                owner = null;
            }
            ...
            if (!isClaimed && canSeeSky()) {
                List<EntityPlayer> entities = worldObj.getEntitiesWithinAABB(
                    EntityPlayer.class,
                    AxisAlignedBB
                        .getBoundingBox(xCoord - 4, yCoord - 1, zCoord - 4, xCoord + 5, yCoord + 2, zCoord + 5));
                        ...
                        if (height >= 1) {
                            MainRegistry.logger.info(
                                "Locking flag " + xCoord
                                    ...
                                    + " "
                                    + zCoord
                                    + " for being hoisted! (captured and raised!)");
                            isClaimed = true;
                            height = 1;
                            this.markDirty();
                        }
            ...
            if (prev == 1F && height != 1F) {
                this.worldObj
                    .playSoundEffect(this.xCoord, this.yCoord, this.zCoord, "hfr:block.flagCapture", 100.0F, 1.0F);

                MainRegistry.logger.info(
                    "Unlocking flag " + xCoord + " " + yCoord + " " + zCoord + " for being unhoisted! (captured!)");

                if (owner != null) owner.notifyCapture(worldObj, xCoord, zCoord, "flags");
            }

            if (prevC != owner) this.worldObj
                .playSoundEffect(this.xCoord, this.yCoord, this.zCoord, "hfr:block.flagChange", 3.0F, 1.0F);
            ...
            if (!canSeeSky()) {
                ...
                if (isClaimed) {
                    MainRegistry.logger
                        .info("Unlocking flag " + xCoord + " " + yCoord + " " + zCoord + " for being obstructed!");
                }

                isClaimed = false;
                setOwner(null);

                if (height >= speed * 2) height -= speed * 2;
            } else if (owner != null) {
                // generateClaim();
            }
    ...
    public void processGauge(int val, int id) {
        ...
        switch (id) {
            ...
            case 3:
                ...
                break;
            case 4:
                prestige = Float.intBitsToFloat(val);
                break;
            case 5:
                prestigeReq = Float.intBitsToFloat(val);
                break;
        }
    }
    ...
    public static float getGenRateFromMode(int mode) {
        ...
        if (mode != 0) return Clowder.flagRate;
    ...
    public static float getCostFromMode(int mode) {
        ...
        if (mode == 1) return Clowder.flagReq * 3;
        if (mode == 2) return Clowder.flagReq * 2;
        if (mode == 3) return Clowder.flagReq * 1;
    ...
    public void setOwner(Clowder c) {
        ...
        if (owner != c) MainRegistry.logger.info(
            "Changing owner for flag " + xCoord
                + " "
                + yCoord
                + " "
                + zCoord
    ...
    public void setMode(int mode) {
        ...
        if (owner.getPrestigeReq() - beforeCost + afterCost > owner.getPrestige()) return;
    ...
    public boolean canSeeSky() {
        ...
        for (int i = -2; i <= 2; i++) for (int j = -2; j <= 2; j++)
            ...
            if (!worldObj.canBlockSeeTheSky(xCoord + i, yCoord + 1, zCoord + j)
                || !worldObj.getBlock(xCoord + i, yCoord - 1, zCoord + j)
                    .isSideSolid(worldObj, xCoord + i, yCoord - 1, zCoord + j, UP))
                return false;

        if (yCoord < 45) return false;
        if (yCoord > 100) return false;
                ...
                if (owner == null) {
                    MainRegistry.logger.info(
                        "Owner (" + id
                            ...
                            + " "
                            + zCoord
                            + " was saved NN but finalized as null!");
                }
            }

            if (owner == null && !id.isEmpty()) MainRegistry.logger.info(
                "Owner (" + id
                    + ") of flag "
                    + xCoord
                    + " "
                    ...
                    + " "
                    + zCoord
                    + " was set in NBT but not found in te clowder list!");
        }
