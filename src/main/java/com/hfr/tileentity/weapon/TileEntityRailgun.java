...
public class TileEntityRailgun extends TileEntity implements ISidedInventory, IEnergyHandler {
    ...
    public EnergyStorage storage = new EnergyStorage(
        MainRegistry.railgunBuffer,
        MainRegistry.railgunBuffer / 100,
        MainRegistry.railgunBuffer / 100);
        ...
        if (!worldObj.isRemote) {
            ...
            if (slots[0] != null && slots[0].getItem() instanceof IEnergyContainerItem) {
                ...
                int extract = (int) Math
                    .min(storage.getMaxEnergyStored() - storage.getEnergyStored(), item.getEnergyStored(slots[0]));
            ...
            PacketDispatcher.wrapper
                .sendToAll(new AuxElectricityPacket(xCoord, yCoord, zCoord, storage.getEnergyStored()));
    ...
    public boolean setAngles(boolean miss) {
        ...
        if (slots[1] != null
            && (slots[1].getItem() == ModItems.designator || slots[1].getItem() == ModItems.designator_range
                || slots[1].getItem() == ModItems.designator_manual)
            ...
            if (vec.lengthVector() < 1 || vec.lengthVector() > 9000) return false;

            double yawUpper = vec.xCoord * unit.xCoord/* + vec.zCoord * unit.zCoord */; // second side falls away since
                                                                                        // unit.z is always 0
            double yawLower = vec.lengthVector()/* * unit.lengthVector() */; // second side falls away since unit always
                                                                             // has length 1
            ...
            if (vec.zCoord > 0) newYaw = 0 - (float) (yaw * 180D / Math.PI);
    ...
    public boolean canFire() {
        ...
        if (slots[2] != null && slots[2].getItem() == ModItems.charge_railgun
            && storage.getEnergyStored() >= required) {
            return true;
        }
