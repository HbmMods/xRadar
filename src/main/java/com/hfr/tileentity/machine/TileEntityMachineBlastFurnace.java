...
public class TileEntityMachineBlastFurnace extends TileEntityMachineBase {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            while (slots[6] != null && slots[6].getItem() == Item.getItemFromBlock(Blocks.coal_block)
                && fuel <= maxFuel - coalValue * 10) {
                this.decrStackSize(6, 1);
                fuel += coalValue * 10;
            }
            ...
            if (operational()) {
                ...
                if (owner != o.owner) {
                    ...
                    if (o != null && o.zone == Zone.FACTION) {
                        if (owner != null) owner.addPrestigeGen(-Clowder.tentRate, worldObj);
                        owner = o.owner;
            ...
            if (meter > 0) {
                ...
                if (dir != ForgeDirection.UNKNOWN) {
                    ...
                    worldObj.spawnParticle(
                        "flame",
                        xCoord + 0.5D + dir.offsetX * 0.75D,
                        yCoord + 0.35D,
                        zCoord + 0.5D + dir.offsetZ * 0.75D,
                        0.0D,
                        0.0D,
                        0.0D);
                    worldObj.spawnParticle(
                        "smoke",
                        xCoord + 0.5D + dir.offsetX * 0.85D,
                        yCoord + 0.5D,
                        zCoord + 0.5D + dir.offsetZ * 0.85D,
                        0.0D,
                        0.0D,
                        0.0D);
                    // worldObj.spawnParticle("smoke", xCoord + 0.5D + dir.offsetX * 0.85D, yCoord + 1.35D, zCoord +
                    // 0.5D + dir.offsetZ * 0.85D, dir.offsetX * 0.1D, 0.0D, dir.offsetZ * 0.1D);
                }

                float y = 0.5F + meter * 4F;
                worldObj.spawnParticle(
                    "smoke",
                    xCoord + 0.5D + this.worldObj.rand.nextGaussian() * 0.25D,
                    yCoord + y,
                    zCoord + 0.5D + this.worldObj.rand.nextGaussian() * 0.25D,
                    0.0D,
                    0.0D,
                    0.0D);
            }
        ...
        if (operational()) {
            ...
            if (worldObj.isRemote) meter = this.meter;

            List<Entity> mobs = worldObj.getEntitiesWithinAABB(
                Entity.class,
                AxisAlignedBB.getBoundingBox(xCoord, yCoord, zCoord, xCoord + 1, yCoord + 0.5 + meter * 4, zCoord + 1));
    ...
    public void processGauge(int val, int id) {
        ...
        switch (id) {
            ...
            case 1:
                ...
                break;
            case 2:
                meter = Float.intBitsToFloat(val);
                break;
        }
    }
    ...
    public float getMeter() {
        ...
        if (fuel > 0) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                if ((slots[i] != null && (slots[i].getItem() == Items.iron_ingot
                    || slots[i].getItem() == Item.getItemFromBlock(Blocks.iron_ore)))) content += slots[i].stackSize;
            }
    ...
    public boolean hasIron() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if ((slots[i] != null && (slots[i].getItem() == Items.iron_ingot
                || slots[i].getItem() == Item.getItemFromBlock(Blocks.iron_ore)))) return true;
        }
    ...
    public void remIron() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if ((slots[i] != null && (slots[i].getItem() == Items.iron_ingot
                || slots[i].getItem() == Item.getItemFromBlock(Blocks.iron_ore)))) {
    ...
    public boolean hasSpace() {
        ...
        for (int i = 3; i < 6; i++) {
            ...
            if ((slots[i] == null || (slots[i] != null && slots[i].getItem() == ModItems.ingot_steel
                && slots[i].stackSize < slots[i].getMaxStackSize()))) return true;
        }
    ...
    public void addSteel() {
        ...
        for (int i = 3; i < 6; i++) {
            ...
            if (slots[i] != null && slots[i].getItem() == ModItems.ingot_steel
    ...
    public void writeToNBT(NBTTagCompound nbt) {
        ...
        if (owner != null) nbt.setString("owner", owner.name);
