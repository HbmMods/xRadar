...
public class TileEntityWaterWheel extends TileEntity implements IEnergyProvider {
    ...
    public EnergyStorage storage = new EnergyStorage(
        MainRegistry.waterwheelProduction * 10,
        MainRegistry.waterwheelProduction,
        MainRegistry.waterwheelProduction);
    ...
    public boolean operational() {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                ...
                if (worldObj.getBlock(xCoord + dir.offsetX * j, yCoord + i, zCoord + dir.offsetZ * j)
    ...
    public boolean hasWater(int x, int y, int z) {
        ...
        if ((worldObj.getBlock(x, y, z) == Blocks.water || worldObj.getBlock(x, y, z) == Blocks.flowing_water)
            && worldObj.getBlockMetadata(x, y, z) > 0) return true;
