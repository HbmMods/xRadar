...
public class TileEntityMachineBuilder extends TileEntityMachineBase {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
             ...
             * SchematicPronter.pront(worldObj, xCoord, yCoord, zCoord,
             * MainRegistry.schems.get(worldObj.rand.nextInt(MainRegistry.schems.size())));
             */

            if (preview != null) {
                if (ox > 1) ox = 1;
                if (oz > 1) oz = 1;
                if (ox < -preview.width) ox = -preview.width;
                if (oz < -preview.length) oz = -preview.length;
            } else {
                ox = 1;
                oz = 1;
            }
            ...
            PacketDispatcher.wrapper.sendToAllAround(
                new BuilderPacket(xCoord, yCoord, zCoord, ox, oz),
    ...
    public boolean doesIntersect() {
        ...
        if (ox <= 0 && oz <= 0 && ox + preview.width > 0 && oz + preview.length > 0) return true;
    ...
    public void construct(Schematic schem) {
        ...
        if (slots[0] != null && slots[0].getItem() == ModItems.wrench && slots[0].stackSize >= i) {
            ...
            preview = null;
            PacketDispatcher.wrapper.sendToAllAround(
                new AuxGaugePacket(xCoord, yCoord, zCoord, 0, 0),
                new TargetPoint(worldObj.provider.dimensionId, xCoord, yCoord, zCoord, 150));
