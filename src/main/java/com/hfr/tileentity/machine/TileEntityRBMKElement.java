...
public class TileEntityRBMKElement extends TileEntityMachineBase implements IFluidHandler {
    ...
    public void processGauge(int val, int id) {
        switch (id) {
            case 0:
                ...
                break;
            case 1:
                water.getFluid().amount = val;
                break;
            case 2:
                steam.getFluid().amount = val;
                break;
        }
    }
    ...
    public void irradiate(ForgeDirection dir, float reactivity) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            if (b == ModBlocks.block_boron) return;

            if (b == ModBlocks.block_graphite) {
                adjusted = origin * 2;

                // one side can release an unadjusted amount of up to 75 at max (exactly 50 would be required to keep
                // the reaction stable)
    ...
    public void readFromNBT(NBTTagCompound nbt) {
        ...
        if (water.getFluid() == null) water.setFluid(new FluidStack(FluidRegistry.WATER, 0));

        if (steam.getFluid() == null) steam.setFluid(new FluidStack(FluidHandler.STEAM, 0));
    }
