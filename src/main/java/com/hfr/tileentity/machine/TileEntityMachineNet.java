...
public class TileEntityMachineNet extends TileEntity implements ISidedInventory {
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            if (operational() && hasSpace() && slots[4] == null) {
                ...
                if (b == BiomeGenBase.ocean || b == BiomeGenBase.deepOcean) fishrate = MainRegistry.superFishrate;

                if (b == BiomeGenBase.river) fishrate = MainRegistry.goodFishrate;

                if (b == BiomeGenBase.desert || b == BiomeGenBase.desertHills
                    || b == BiomeGenBase.mesa
                    || b == BiomeGenBase.mesaPlateau
                    || b == BiomeGenBase.mesaPlateau_F
                    || b == BiomeGenBase.extremeHills
                    || b == BiomeGenBase.savanna
                    || b == BiomeGenBase.savannaPlateau
                    || b == BiomeGenBase.extremeHillsEdge
                    || b == BiomeGenBase.extremeHillsPlus
                    || b == BiomeGenBase.hell) fishrate = MainRegistry.crapFishrate;
                ...
                if (worldObj.rand.nextInt(fishrate * 20) == 0) {
                    ...
                    if (worldObj.rand.nextInt(100) < MainRegistry.whaleChance) fish = ModItems.whale_meat;
                ...
                if (worldObj.rand.nextInt(MainRegistry.jamRate * 20) == 0) {
                    ...
                    if (i < 5) slots[4] = new ItemStack(Items.stick).setStackDisplayName("Driftwood");
                    else if (i < 7) slots[4] = new ItemStack(Blocks.sapling);
                    else if (i < 9) slots[4] = new ItemStack(Blocks.waterlily);
                    else slots[4] = new ItemStack(Items.bone).setStackDisplayName("Rattle Me Bones");
                }
            }
    ...
    public boolean operational() {
        ...
        for (int i = -2; i <= 2; i++)
            for (int j = -2; j <= 2; j++) if (worldObj.getBlock(xCoord + i, yCoord, zCoord + j)
                .getMaterial() != Material.air && !(i == 0 && j == 0)
                || !worldObj.canBlockSeeTheSky(xCoord + i, yCoord, zCoord + j)) return false;

        for (int x = -2; x <= 2; x++) for (int y = -3; y < 0; y++)
            for (int z = -2; z <= 2; z++) if (worldObj.getBlock(xCoord + x, yCoord + y, zCoord + z)
    ...
    public boolean hasSpace() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if ((slots[i] == null || (slots[i] != null && slots[i].getItem() == Items.fish
                && slots[i].stackSize < slots[i].getMaxStackSize()))
                || (slots[i] != null && slots[i].getItem() == ModItems.whale_meat
                    && slots[i].stackSize < slots[i].getMaxStackSize()))
                return true;
        }
