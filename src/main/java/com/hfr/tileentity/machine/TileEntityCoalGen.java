...
public class TileEntityCoalGen extends TileEntityMachineBase implements IEnergyProvider {
    ...
    public EnergyStorage storage = new EnergyStorage(
        MainRegistry.coalgenProduction * 1000,
    ...
    public void updateEntity() {
        ...
        if (!worldObj.isRemote) {
            ...
            // if the furnace isn't burning, there's still room or energy and the fuel item is combustable, the item
            // will be ignited
            if (burnTime == 0 && storage.getEnergyStored() < storage.getMaxEnergyStored()
                && slots[0] != null
                && TileEntityFurnace.getItemBurnTime(slots[0]) > 0) {
            ...
            if (slots[1] != null && slots[1].getItem() instanceof IEnergyContainerItem) {
                ...
                int receive = (int) Math
                    .min(storage.getEnergyStored(), item.getMaxEnergyStored(slots[1]) - item.getEnergyStored(slots[1]));
