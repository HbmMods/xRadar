...
public class TileEntityForceField extends TileEntity implements ISidedInventory, IEnergyHandler {
    ...
    public void updateEntity() {
        ...
        if (cooldown > 0) {
            ...
            if (health < maxHealth) health += maxHealth / 100;
        ...
        if (!worldObj.isRemote) {
            if (storage.getEnergyStored() < powerCons) storage.setEnergyStored(0);
        }
    ...
    private int impact(Entity e) {
        ...
        if (e instanceof EntityRailgunBlast) return 1000000;

        if (e instanceof EntityMissileBaseSimple) {

            ((EntityMissileBaseSimple) e).onForceImpact();

            if (((EntityMissileBaseSimple) e).getIsBreaching()) return 1000000;
            else return 0;
        }
        ...
        try {
            ...
            if (bulletO != null) {
                ...
                Field fdmg = ReflectionHelper.findField(
                    cl.cast(bulletO)
                        .getClass()
                        .getSuperclass(),
                    "damageVsLiving");
    ...
    private void damage(int ouch) {
        ...
        if (ouch >= (this.maxHealth / 250D)) blink = 5;
        ...
        for (Entity o : list) {
            ...
            if ((o instanceof EntityPlayer) && !(o instanceof EntityMissileAntiBallistic)) {
                ...
                double dist = Math.sqrt(
                    Math.pow(xCoord + 0.5 - entity.posX, 2) + Math.pow(yCoord + 0.5 - entity.posY, 2)
                        + Math.pow(zCoord + 0.5 - entity.posZ, 2));
                ...
                if (!oLegacy.contains(entity) && !iLegacy.contains(entity)) {
                    ...
                    if (oLegacy.contains(entity) && !out) {
                        Vec3 vec = Vec3.createVectorHelper(
                            xCoord + 0.5 - entity.posX,
                            yCoord + 0.5 - entity.posY,
                        ...
                        double mo = Math.sqrt(
                            Math.pow(entity.motionX, 2) + Math.pow(entity.motionY, 2) + Math.pow(entity.motionZ, 2));
                        ...
                        if (iLegacy.contains(entity) && out) {
                            ...
                            double mo = Math.sqrt(
                                Math.pow(entity.motionX, 2) + Math.pow(entity.motionY, 2)
