...
public class TileEntityMachineRadar extends TileEntity implements IEnergyHandler {
    ...
    public EnergyStorage storage = new EnergyStorage(
        MainRegistry.coalgenProduction * 1200,
        MainRegistry.coalgenProduction * 10,
        MainRegistry.coalgenProduction * 10);
    ...
    public void updateEntity() {
        ...
        if (this.yCoord < MainRegistry.radarAltitude) return;
        ...
        if (!worldObj.isRemote) PacketDispatcher.wrapper.sendToAllAround(
            new AuxElectricityPacket(xCoord, yCoord, zCoord, storage.getEnergyStored()),
            new TargetPoint(this.worldObj.provider.dimensionId, xCoord, yCoord, zCoord, 25));
    ...
    private void allocateMissiles() {
        ...
        List<Entity> list = worldObj.getEntitiesWithinAABBExcludingEntity(
            ...
            AxisAlignedBB.getBoundingBox(
                xCoord + 0.5 - MainRegistry.radarRange,
                0,
                zCoord + 0.5 - MainRegistry.radarRange,
                xCoord + 0.5 + MainRegistry.radarRange,
                5000,
                zCoord + 0.5 + MainRegistry.radarRange));
        ...
        for (Entity e : list) {
            ...
            if (e.posY >= this.yCoord + MainRegistry.radarBuffer) {
                ...
                if (mode == 0 || mode == 2) {
                    if (e instanceof EntityPlayer) nearbyMissiles.add(
                        new RadarEntry((int) e.posX, (int) e.posY, (int) e.posZ, ((EntityPlayer) e).getDisplayName()));
                }
                if (mode == 0 || mode == 1) {
                    if (e instanceof EntityMissileBaseAdvanced) nearbyMissiles.add(
                        new RadarEntry(
                            (int) e.posX,
                            (int) e.posY,
                            (int) e.posZ,
                            "Tier " + (((EntityMissileBaseAdvanced) e).getMissileType() + 1) + " Missile"));
                        ...
                        nearbyMissiles.add(
                            new RadarEntry(
                                ...
                                (int) e.posY,
                                (int) e.posZ,
                                "Tier " + (((EntityMissileBaseSimple) e).getMissileType() + 1)
                                    + " Missile (Ascending)"));

                    else if (e instanceof EntityMissileBaseSimple) nearbyMissiles.add(
                        new RadarEntry(
                            (int) e.posX,
                            (int) e.posY,
                            (int) e.posZ,
                            "Tier " + (((EntityMissileBaseSimple) e).getMissileType() + 1) + " Missile"));

                    else if (e instanceof EntityMissileAntiBallistic) nearbyMissiles
                        .add(new RadarEntry((int) e.posX, (int) e.posY, (int) e.posZ, "Anti-Ballistic Missile"));
    ...
    public int getRedPower() {
        ...
        if (!nearbyMissiles.isEmpty()) {
            ...
            for (int i = 0; i < nearbyMissiles.size(); i++) {
                ...
                if (j.name.equals("Anti-Ballistic Missile") || j.name.endsWith("(Ascending)")) continue;
    ...
    private void sendMissileData() {
         ...
         * PacketDispatcher.wrapper.sendToAll(new TERadarPacket(xCoord, yCoord, zCoord, e.posX, e.posY, e.posZ,
         * e.name));
         * }
         */

        PacketDispatcher.wrapper
            .sendToAll(new TESRadarPacket(nearbyMissiles.toArray(new RadarEntry[0]), xCoord, yCoord, zCoord, mode));
    }
