...
public class TileEntityMachineSawmill extends TileEntityMachineBase implements IEnergyHandler {
        ...
        if (!worldObj.isRemote) {
            ...
            if (slots[1] != null && slots[1].getItem() instanceof IEnergyContainerItem) {
                ...
                int extract = (int) Math
                    .min(storage.getMaxEnergyStored() - storage.getEnergyStored(), item.getEnergyStored(slots[1]));
            ...
            if (progress > 0) {
                ...
                switch (getBlockMetadata() - 10) {
                    case 2:
                        vec.rotateAroundY((float) Math.toRadians(0));
                        break;
                    case 4:
                        vec.rotateAroundY((float) Math.toRadians(90));
                        break;
                    case 3:
                        vec.rotateAroundY((float) Math.toRadians(180));
                        break;
                    case 5:
                        vec.rotateAroundY((float) Math.toRadians(270));
                        break;
                }
    ...
    private void process(ItemStack output) {
        ...
        if (slots[3] == null) {
            ...
            if (slots[3].stackSize < slots[3].getMaxStackSize()) slots[3].stackSize++;
        }
        ...
        if (worldObj.rand.nextInt(3) == 0) {
            if (slots[4] == null) {
                ...
                if (slots[4].stackSize < slots[4].getMaxStackSize()) slots[4].stackSize++;
            }
        }
    }
    ...
    public boolean hasSpace(ItemStack output) {
        ...
        if (slots[2].getItem() == output.getItem() && slots[2].getItemDamage() == output.getItemDamage()
            && slots[2].stackSize + output.stackSize <= slots[2].getMaxStackSize()) return true;
