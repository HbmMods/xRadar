...
public class TileEntityChlorineSeal extends TileEntity {
    ...
    private void spread(int x, int y, int z, int index) {
        ...
        if (b.isReplaceable(worldObj, x, y, z) && !b.getMaterial()
            .isLiquid()) worldObj.setBlock(x, y, z, ModBlocks.chlorine_gas);

        b = worldObj.getBlock(x, y, z);

        if (b != ModBlocks.chlorine_gas && b != ModBlocks.vent_chlorine_seal) return;
