...
public class TileEntityFoundry extends TileEntityMachineBase {
    ...
    private boolean canProcess(Item target, float cost) {
        ...
        if (steel < cost) return false;

        if (slots[2] == null) return true;

        if (slots[2].getItem() == target && slots[2].stackSize < slots[2].getMaxStackSize()) return true;
    ...
    private float getSteel(ItemStack stack) {
        ...
        Float steel = recipes.get(
            stack.getItem()
                .getUnlocalizedName());

        if (steel == null) return 0.0F;
