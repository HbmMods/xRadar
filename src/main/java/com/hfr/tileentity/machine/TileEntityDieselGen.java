...
public class TileEntityDieselGen extends TileEntityMachineBase implements IEnergyProvider {
    ...
    public EnergyStorage storage = new EnergyStorage(
        MainRegistry.dieselProduction * 100,
        MainRegistry.dieselProduction * 10,
        MainRegistry.dieselProduction * 10);
        ...
        if (!worldObj.isRemote) {
            ...
            if (slots[0] != null && slots[0].getItem() == ModItems.canister_diesel && diesel + 1000 <= maxDiesel) {
                ...
                if (slots[1] == null) {
                    ...
                    diesel += 1000;
                } else if (slots[1].getItem() == ModItems.canister_empty
            ...
            if (slots[2] != null && slots[2].getItem() instanceof IEnergyContainerItem) {
                ...
                int receive = (int) Math
                    .min(storage.getEnergyStored(), item.getMaxEnergyStored(slots[2]) - item.getEnergyStored(slots[2]));
    ...
    public void processGauge(int val, int id) {
        ...
        if (id == 0) storage.setEnergyStored(val);
        if (id == 1) diesel = val;
    }
