...
public class TileEntityMachineMarket extends TileEntity implements ISidedInventory {
            ...
            for (EntityPlayer player : list) {
                ...
                for (int i = 0; i < data.stocks.size(); i++) {
                    ...
                    PacketDispatcher.wrapper.sendTo(
                        new StockPacket(stock.name, stock.shortname, stock.value, shares, i),
    ...
    public void buyStock(Stock stock, String player) {
        ...
        for (int i = 0; i < 3; i++)
            if (slots[i] != null && slots[i].getItem() == ModItems.coin) coins += slots[i].stackSize;
        ...
        if (stocks >= MainRegistry.stockCap) return;
    ...
    public void sellStock(Stock stock, String player) {
        ...
        for (int i = 3; i < 6; i++) {
            ...
            if (slots[i] != null && slots[i].getItem() == ModItems.coin) space += 64 - slots[i].stackSize;

            if (slots[i] == null) space += 64;
        }
