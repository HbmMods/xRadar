...
public class TileEntityMachineFactory extends TileEntityMachineBase implements IEnergyReceiver {
    ...
    public EnergyStorage storage = new EnergyStorage(
        MainRegistry.factoryConsumption * 100,
        MainRegistry.factoryConsumption * 10,
        MainRegistry.factoryConsumption * 10);
        ...
        if (!worldObj.isRemote) {
            ...
            if (operational() && hasSpace() && slots[4] == null) {
                ...
                if (worldObj.rand.nextInt(MainRegistry.factoryRate * 20) == 0) {
                    ...
                    for (int i = 0; i < 4; i++) {
                        if (slots[i] == null) {
                            ...
                            if (slots[i].getItem() == ModItems.cog && slots[i].stackSize < slots[i].getMaxStackSize()) {
                                slots[i].stackSize++;
                                break;
                            }
                    }
                ...
                if (worldObj.rand.nextInt(MainRegistry.factoryJamRate * 20) == 0) {
                    ...
                    if (i < 5) slots[4] = new ItemStack(Items.paper).setStackDisplayName("Worker Strike");
                    else if (i < 7) slots[4] = new ItemStack(Items.skull).setStackDisplayName("Workplace Accident");
                    else if (i < 9) slots[4] = new ItemStack(Items.slime_ball).setStackDisplayName("Chemical Spill");
                    else slots[4] = new ItemStack(Items.potato).setStackDisplayName("Communist Takeover");
                }
            }
    ...
    public boolean operational() {
        ...
        for (int i = -4; i <= 4; i++)
            for (int j = -4; j <= 4; j++) if (worldObj.getBlock(xCoord + i, yCoord + 4, zCoord + j)
                .getMaterial() != Material.air && !(i == 0 && j == 0)
                || !worldObj.canBlockSeeTheSky(xCoord + i, yCoord + 4, zCoord + j)) return false;

        for (int x = -4; x <= 4; x++) for (int z = -4; z <= 4; z++)
            if (!(worldObj.getBlock(xCoord + x, yCoord - 1, zCoord + z) instanceof BlockSpeedy)) return false;

        if (storage.getEnergyStored() < MainRegistry.factoryConsumption) return false;

        if (slots[4] != null) return false;
    ...
    public boolean hasSpace() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            if ((slots[i] == null || (slots[i] != null && slots[i].getItem() == ModItems.cog
